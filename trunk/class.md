...
[*Note 4*: Standard-layout classes are useful for communicating with
...
[*Note 5*: Aggregates of class type are described in 
...
- when the *exception-declaration* of a *handler* [[except.handle]]
  ...
  the *exception-declaration*. \[*Note 3*: There cannot be a move from
...
yields `true`. The return value is a copy of `vᵢ` if such an index
exists and `static_cast<R>(std::strong_ordering::equal)` otherwise.
