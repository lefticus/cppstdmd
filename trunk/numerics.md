...
- The following computations are applied to the elements of the V
  ...
  where:
  - \mullo(`a`, `b`, `w`) is the low half of the modular multiplication
    of `a` and `b`: (`a` \cdot `b`) \mod 2^w,
  - \mulhi(`a`, `b`, `w`) is the high half of the modular multiplication
    of `a` and `b`: (\left\lfloor (`a` \cdot `b`) / 2^w \right\rfloor),
...
*Effects:* Sets the K₀ element of sequence K to `value` \mod 2^w. All
elements of sequences X and K (except K₀) are set to `0`. The value of i
is set to n - 1.
