...
%%
%% Example: \itcorr (Issue #74) was being stripped in 3 different places:
%% cpp-tables.lua, cpp-common.lua, cpp-grammar.lua. Moving it here
%% eliminated duplication and fixed bugs in one place.
%%--------------------------------------------------

%%--------------------------------------------------
%% Category A: Simple Text Substitutions (21 macros)
...
\renewcommand{\newline}{ }    % Strip line breaks - markdown doesn't support mid-table breaks (Issue #27)
\renewcommand{\@}{}           % Sentence spacing control (LaTeX typography)
\renewcommand{\textregistered}{®}  % Registered trademark symbol (Issue #14)
\renewcommand{\Cpp}{C++}
...
%%--------------------------------------------------
%% Category B: Code Formatting (9 macros)
...
%% \terminal{} - REMOVED: needs special handling in cpp-grammar.lua for BNF terminals (Issue #27)
...
\renewcommand{\libnoheader}[1]{\texttt{<#1>}}  % Reserved/absent header name (used in n4861+) - Issue #80
...
%%--------------------------------------------------
%% Category C: Emphasis Formatting (4 macros)
...
%% \exposconceptnc{} - REMOVED: context-dependent (needs code+italic), moved to cpp-macros.lua - Issue #49
\renewcommand{\seebelownc}[1]{\textit{see below}}  % See below (no correction) - Issue #49 - required arg consumes {}
...
\renewcommand{\techterm}[1]{\textit{#1}}  % Technical term (obsolete macro used in n3337-n4659) - Issue #79
\renewcommand{\oldconcept}[1]{\textit{Cpp17#1}}
\renewcommand{\doccite}[1]{\textit{#1}}
\renewcommand{\exposconceptx}[2]{\texttt{\textit{#1}}}  % Exposition-only concept with index - Issue #68
...
\renewcommand{\notes}{\textit{Remarks:} }  % Alias for \remarks (obsolete macro used in n3337-n4140) - Issue #80
\renewcommand{\realnotes}{\textit{Notes:} }  % Similar to \realnote but plural (obsolete macro used in n3337-n4140) - Issue #80
...
%% Code formatting for C preprocessor macros (298 occurrences)
\renewcommand{\libmacro}[1]{\texttt{#1}}

%% Grammar non-terminals (47 occurrences)
\renewcommand{\fmtnontermdef}[1]{\textit{#1}}  % Format string grammar (33×)
\renewcommand{\renontermdef}[1]{#1::}   % Regex grammar (7×) - just term+:: (like \locnontermdef)
\renewcommand{\defexposconceptnc}[1]{\textit{#1}}  % Exposition-only concepts (29×)

%% Math state variables: \state{X}{Y} → X_Y with subscripts (27 occurrences)
\renewcommand{\state}[2]{#1\ensuremath{_{#2}}}

%%--------------------------------------------------
%% Category M: BNF Typography Macros (Issue #74)
%% Pure typesetting commands with no semantic meaning
%%--------------------------------------------------
\renewcommand{\BnfNontermshape}{}  % BNF font styling (small, gray, sans-serif) - strip for markdown
\renewcommand{\itcorr}[1][]{}      % Italic correction spacing - strip for markdown (33+ occurrences)
\renewcommand{\locnontermdef}[1]{#1:}  % Locale BNF nonterminal (13× in localization.md) - just term+colon

%%--------------------------------------------------
%% TOTAL: 94 simplified macros (was 93, added 1 for Issue #27)
%% Macros cover ~75% of macro usage in the C++ standard source
%%
...
%%  - \exposid{}, \exposidnc{}, \exposconceptnc{} - context-dependent (need code+italic) - Issue #49
