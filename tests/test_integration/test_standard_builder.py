...
class TestStandardBuilder:
    ...
    def test_extract_stable_name_from_tex(self, converter, draft_repo):
        ...
        for filename, expected_stable_name in test_cases:
            ...
            if tex_file.exists():
                ...
                assert (
                    stable_name == expected_stable_name
    ...
    def test_build_subset_of_chapters(self, converter, draft_repo):
        ...
        try:
            ...
            content, chapters = builder.build_full_standard(converter, output_file, verbose=True)
            ...
            assert (
                len(chapters) == 3
            ), f"Should have converted exactly 3 chapters, got {len(chapters)}"
            assert chapters == [
                "intro",
                "lex",
                "basic",
            ], f"Should have converted intro, lex, basic, got {chapters}"
            ...
            assert separator_count == len(
                chapters
            ), f"Should have {len(chapters)} separators for {len(chapters)} chapters with TOC, got {separator_count}"

            # Verify link definitions section exists
            assert (
                "<!-- Link reference definitions -->" in content
            ), "Should have link definitions section"
            ...
            assert (
                len(refs) > 10
            ), f"Should have many cross-reference definitions, found {len(refs)}"
    ...
    def test_build_full_standard(self, converter, draft_repo):
        ...
        try:
            ...
            assert (
                len(chapters) >= 37
            ), f"Should have converted at least 37 chapters, got {len(chapters)}"
            ...
            assert separator_count == len(
                chapters
            ), f"Should have {len(chapters)} separators for {len(chapters)} chapters with TOC"
    ...
    def test_build_separate_chapters_subset(self, converter, draft_repo):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cross_file_links_intro = re.findall(
                r"\[([a-z]+\.[a-z.]+)\]:\s*([a-z]+)\.md#\1", intro_content
            )
            ...
            if front_file.exists():
                ...
                toc_cross_file_links = re.findall(
                    r"\[([^\]]+)\]\(([a-z]+)\.md#([a-z.]+)\)", front_content
                )
    ...
    def test_build_separate_chapters_full(self, converter, draft_repo):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                len(output_files) >= 37
            ), f"Should have created at least 37 files, got {len(output_files)}"
            ...
            for output_file in output_files:
                ...
                cross_links = re.findall(
                    r"\[[a-z]+\.[a-z.]+\]:\s*[a-z]+\.md#[a-z]+\.[a-z.]+", content
                )
