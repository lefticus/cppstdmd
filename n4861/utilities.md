...
- `t_1.*f` when `N == 1` and `f` is a pointer to data member of a class
  `T` and `is_base_of_v<T, remove_reference_t<decltype(t_1)>>` is
  `true`;
- `t_1.get().*f` when `N == 1` and `f` is a pointer to data member of a
  class `T` and `remove_cvref_t<decltype(t_1)>` is a specialization of
  `reference_wrapper`;
- `(*t_1).*f` when `N == 1` and `f` is a pointer to data member of a
  class `T` and `t_1` does not satisfy the previous two items;
