...
The expression inside `explicit` evaluates to `false` if and only if the
...
Subtracts the scalar value `rhs` from the real part of the complex value
`*this` and stores the result in the real part of `*this`, leaving the
imaginary part unchanged.
...
Extracts a complex number `x` of the form: `u`, `(u)`, or `(u,v)`, where
`u` is the real part and `v` is the imaginary part\[istream.formatted\].
...
The `complex` value corresponding to a complex number whose magnitude is
...
Sets the values of $X_{-r}, \dotsc, X_{-1}$, in that order, as specified
below. If $X_{-1}$ is then 0, sets c to 1; otherwise sets c to 0.
...
Does nothing if `begin == end`. Otherwise, with $s = \texttt{v.size()}$
and $n = \texttt{end} - \texttt{begin}$, fills the supplied range
$[\texttt{begin},\texttt{end})$ according to the following algorithm in
...
Exactly $k = \max(1, \left\lceil b / \log_2 R \right\rceil)$ invocations
of `g`, where b
...
`t` and `p` correspond to the respective parameters of the distribution.
...
`k` and `p` correspond to the respective parameters of the distribution.
...
The value of the `mean` parameter with which the object was constructed.
...
The value of the `beta` parameter with which the object was constructed.
...
`m` and `s` correspond to the respective parameters of the distribution.
...
`m` and `n` correspond to the respective parameters of the distribution.
...
Constructs a `discrete_distribution` object with probabilities given by
the formula above.
...
Constructs a `discrete_distribution` object with probabilities given by
the formula above, using the following values: If $\texttt{nw} = 0$, let
w₀ = 1. Otherwise, let
...
A `vector<double>` whose `size` member returns n and whose `operator[]`
member returns pₖ when invoked with argument k for
$k = 0, \dotsc, n - 1$.
...
Constructs a `piecewise_constant_distribution` object with parameters as
specified above.
...
$\texttt{operator[]}$ member returns bₖ when invoked with argument k for
$k = 0, \dotsc, n$.
...
$\texttt{operator[]}$ member returns $\rho_k$ when invoked with argument
k for $k = 0, \dotsc, n - 1$.
...
Constructs a `valarray` that has length `n`. The values of the elements
of the array are initialized with the first `n` values pointed to by the
first argument.
...
`*this` obtains the value of `v`. The value of `v` after the assignment
is not specified.
...
operator does not depend on the value of another element in that
left-hand side.
...
array is initialized with the result of applying the indicated operator
to the corresponding element of the array.
...
Each of these operators performs the indicated operation on each of the
...
The appearance of an array on the left-hand side of a compound
assignment does not invalidate references or pointers to the elements of
the array.
...
A `valarray` of length `size()` that is a circular shift of `*this`. If
element zero is taken as the leftmost element, a non-negative value of n
shifts the elements circularly left n places and a negative value of n
shifts the elements circularly right -n places.
...
Changes the length of the `*this` array to `sz` and then assigns to each
element the value of the second argument. Resizing invalidates all
pointers and references to elements in the array.
...
result of applying the indicated operator to the corresponding elements
...
A `valarray` whose length is equal to the length of the array argument.
Each element of the returned array is initialized with the result of
applying the indicated operator to the corresponding element of the
array argument and the non-array argument.
...
The indicated operator can be applied to operands of type `T` and
returns a value of type `bool` or which can be unambiguously implicitly
converted to `bool`.
...
to an operand of type `T`. This function returns a value of type `T` or
...
These functions compute the beta function of their respective arguments
`x` and `y`.
...
of their respective arguments `k`.
...
These functions compute the complete elliptic integral of the third kind
...
These functions compute the cylindrical Neumann functions, also known as
...
kind of their respective arguments `k`, `nu`, and `phi` (`phi` measured
in radians).
...
These functions compute the spherical Bessel functions of the first kind
