...
For the first member function, `Ptr::pointer_to(r)` returns a pointer to
...
If `element_type` is  , the type of `r` is unspecified; otherwise, it is
`element_type&`.
...
updates `ptr` to represent the first possible address of such storage
...
objects nested within *a*, as follows: The object representation of *a*
is the contents of the storage prior to the call to `start_lifetime_as`.
The value of each created object *o* of trivially-copyable type `U` is
...
If `n > 0` is `true`, equivalent to `start_lifetime_as<U>(p)` where `U`
is the type “array of `n` `T`”. Otherwise, there are no effects.

***Returns:***

A pointer to the first element of the created array, if any; otherwise,
...
function starts the lifetime of the array object, but not that of any of
the array elements.
...
Uses `::operator delete`\[new.delete\], but it is unspecified when this
function is called.
...
The actual address of the object or function referenced by `r`, even in
...
`get() == p`. `get_deleter()` returns a reference to the stored deleter.
...
Constructs a `unique_ptr` object which owns `p`, initializing the stored
...
`get() == p`. `get_deleter()` returns a reference to the stored deleter.
If `D` is a reference type then `get_deleter()` returns a reference to
the lvalue `d`.
...
deleter is copy constructed from `u`’s deleter; otherwise, this deleter
is move constructed from `u`’s deleter.
...
`u.get() == nullptr`. `get_deleter()` returns a reference to the stored
deleter that was constructed from `u.get_deleter()`. If `D` is a
reference type then `get_deleter()` and `u.get_deleter()` both reference
...
Otherwise, `E` is a reference type and construction of the deleter from
...
`get()` yields the value `u.get()` yielded before the construction.
`u.get() == nullptr`. `get_deleter()` returns a reference to the stored
deleter that was constructed from `u.get_deleter()`.
...
If `this != addressof(u)`, `u.get() == nullptr`, otherwise `u.get()` is
unchanged.
...
If `E` is not a reference type, assignment of the deleter from an rvalue
of type `E` is well-formed and does not throw an exception. Otherwise,
`E` is a reference type and assignment of the deleter from an lvalue of
type `E` is well-formed and does not throw an exception.
...
\[*Note 8*: This replaces the specification of the primary
template. — *end note*\]
...
\[*Note 9*: This replaces the specification of the primary
template. — *end note*\]
...
and either `T` is `U[N]` and `Y(*)[N]` is convertible to `T*`, or `T` is
`U[]` and `Y(*)[]` is convertible to `T*`. When `T` is not an array
...
owns the pointer `p`. Otherwise, constructs a `shared_ptr` that owns `p`
and a deleter of an unspecified type that calls `delete[] p`. When `T`
is not an array type, enables `shared_from_this` with `p`. If an
exception is thrown, `delete p` is called when `T` is not an array type,
`delete[] p` otherwise.
...
Constructs a `shared_ptr` object that shares ownership with `r` and
stores a copy of the pointer stored in `r`. If an exception is thrown,
...
If `r.get() == nullptr`, equivalent to `shared_ptr()`. Otherwise, if `D`
is not a reference type, equivalent to
...
When `T` is an array type or  , it is unspecified whether this member
...
When `T` is an array type, it is unspecified whether this member
function is declared. If it is declared, it is unspecified what its
return type is, except that the declaration (although not necessarily
the definition) of the function shall be well-formed.
...
`get() != nullptr && i >= 0`. If `T` is `U[N]`, `i < N`.
...
When `T` is not an array type, it is unspecified whether this member
function is declared. If it is declared, it is unspecified what its
return type is, except that the declaration (although not necessarily
the definition) of the function shall be well-formed.
...
The number of `shared_ptr` objects, `*this` included, that share
...
Allocates memory for an object of type `T` (or `U[N]` when `T` is `U[]`,
where `N` is determined from *args* as specified by the concrete
overload). The object is initialized from *args* as specified by the
concrete overload. The `allocate_shared` and
...
`r.get() != nullptr && r.use_count() == 1`, where `r` is the return
value.

***Returns:***

A `shared_ptr` instance that stores and owns the address of the newly
constructed object.

***Throws:***

`bad_alloc`, or an exception thrown from `allocate` or from the
...
A `shared_ptr` to an object of type `T` with an initial value
`T(std::forward<Args>(args)...)`.

***Remarks:***

The `shared_ptr` constructors called by these functions enable
...
A `shared_ptr` to an object of type `U[N]` with a default initial value,
where `U` is `remove_extent_t<T>`.
...
A `shared_ptr` to an object of type `U[N]`, where `U` is
...
A `shared_ptr` to an object of type `T`, where each array element of
...
The expression `static_cast<T*>((U*)nullptr)` is well-formed.
...
The expression `dynamic_cast<T*>((U*)nullptr)` is well-formed. The
expression
...
The expression `const_cast<T*>((U*)nullptr)` is well-formed.
...
The expression `reinterpret_cast<T*>((U*)nullptr)` is well-formed.
...
If `p` owns a deleter `d` of type cv-unqualified `D`, returns
...
Constructs an empty `weak_ptr` object that stores a null pointer value.
...
If `r` is empty, constructs an empty `weak_ptr` object that stores a
null pointer value; otherwise, constructs a `weak_ptr` object that
shares ownership with `r` and stores a copy of the pointer stored in
`r`.
...
Move constructs a `weak_ptr` instance from `r`.
...
`0` if `*this` is empty; otherwise, the number of `shared_ptr` instances
...
`expired() ? shared_ptr<T>() : shared_ptr<T>(*this)`, executed
atomically.
...
Initializes `s` with `smart`, `a` with `std::forward<Args>(args)...`,
and value-initializes `p`. Then, equivalent to:
...
`operator void**()` has not been called on `*this`.
...
`operator Pointer*()` has not been called on `*this`.

***Returns:***

A pointer value `v` such that:
...
Accessing `*v` outside the lifetime of `*this` has undefined behavior.
...
An implementation can call `s.release()`.
...
Allocates storage by calling `do_allocate(bytes, alignment)` and
implicitly creates objects within the allocated region of storage.

***Returns:***

A pointer to a suitable created object\[intro.object\] in the allocated
region of storage.

***Throws:***

What and when the call to `do_allocate` throws.
...
A derived class implementation shall throw an appropriate exception if
...
`p` was returned from a prior call to `allocate(bytes, alignment)` on a
memory resource equal to `*this`, and the storage at `p` has not yet
been deallocated.

***Effects:***

A derived class shall implement this function to dispose of allocated
storage.
...
A derived class shall implement this function to return `true` if memory
allocated from can be deallocated from `other` and vice-versa, otherwise
`false`.
...
If `numeric_limits<size_t>::max() / sizeof(Tp) < n`, throws
...
`p` was allocated from a memory resource `x`, equal to `*memory_rsrc`,
using `x.allocate(n * sizeof(Tp), alignof(Tp))`.

***Effects:***

Equivalent to `memory_rsrc->deallocate(p, n * sizeof(Tp), alignof(Tp))`.
...
Equivalent to: `return memory_rsrc->allocate(nbytes, alignment);`
...
Equivalent to `memory_rsrc->deallocate(p, nbytes, alignment)`.
...
Allocates memory suitable for holding an array of `n` objects of type
`T`, as follows:
...
Equivalent to `deallocate_bytes(p, n*sizeof(T), alignof(T))`.
...
Allocates and constructs an object of type `T`, as follows. Equivalent
to:
...
Construct a `T` object in the storage whose address is represented by
`p` by uses-allocator construction with allocator `*this` and
...
If `r` is non-null, sets the value of the default memory resource
pointer to `r`, otherwise sets the default memory resource pointer to
`new_delete_resource()`.
...
Calling the `set_default_resource` and `get_default_resource` functions
shall not incur a data race. A call to the `set_default_resource`
function shall synchronize with subsequent calls to the
...
`upstream` whenever the pool resource is unable to satisfy a memory
...
Nothing unless `upstream->allocate()` throws. It is unspecified if, or
under what conditions, this constructor calls `upstream->allocate()`.
...
Calls `upstream_resource()->deallocate()` as necessary to release all
allocated memory.
...
The value of the `upstream` argument provided to the constructor of this
object.
...
If the pool selected for a block of size `bytes` is unable to satisfy
the memory request from its own internal data structures, it will call
...
size of at least `bytes`. The size and alignment of the allocated memory
shall meet the requirements for a class derived from
...
Returns the memory at `p` to the pool. It is unspecified if, or under
...
`upstream` is the address of a valid memory resource. `initial_size`, if
specified, is greater than zero.
...
`upstream` is the address of a valid memory resource. `buffer_size` is
no larger than the number of bytes in `buffer`.
...
Calls `upstream_rsrc->deallocate()` as necessary to release all
allocated memory. Resets `current_buffer` and `next_buffer_size` to
...
If the unused space in `current_buffer` can fit a block with the
...
Memory used by this resource increases monotonically until its
destruction.
...
`scoped_allocator_adaptor<OuterAlloc>` if `sizeof...(InnerAllocs)` is
zero; otherwise,  
...
`true_type` if `allocator_traits<A>::propagate_on_container_swap::value`
...
`true_type` if `allocator_traits<A>::is_always_equal::value` is `true`
...
Value-initializes the `OuterAlloc` base class and the `inner` allocator
object.
...
Initializes the `OuterAlloc` base class with
...
Move constructs each allocator within the adaptor with the corresponding
allocator from `other`.
...
A new `scoped_allocator_adaptor` object where each allocator `a1` within
